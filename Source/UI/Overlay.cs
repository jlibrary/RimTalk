        ...
        private void UpdateAndRecalculateCache()
        {
            ...
            try
            {
                ...
                foreach (var message in messages)
                {
                    ...
                    // Find the pawn instance here, ONCE, and store it.
                    var foundPawn = Cache.GetByName(pawnName)?.Pawn;
                    if (foundPawn == null)
                    {
                        foundPawn = Find.CurrentMap?.mapPawns?.AllPawns?.FirstOrDefault(p => p?.Name?.ToStringShort == pawnName);
                    }
                    if (foundPawn == null)
                    {
                        foundPawn = Find.WorldPawns?.AllPawnsAliveOrDead.FirstOrDefault(p => p?.Name?.ToStringShort == pawnName);
                    }
